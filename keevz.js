...
fs.readdir('./src/commands/', (err, files) => {
  ...
  console.log(`[keevz] ${files.length} komut yüklenecek.`);
  files.forEach(f => {
    fs.readdir("./src/commands/" + f, (err2, files2) => {
      files2.forEach(file => {
        let props = require(`./src/commands/${f}/` + file);
        console.log(`[keevz KOMUT] ${props.conf.name} komutu yüklendi!`);
...
client
  ...
  (async () => {
    try {
      console.log('[keevz] Slash Komutlar yükleniyor.');
      await rest.put(
        Routes.applicationCommands(settings.BotClientID),
        { body: slashcommands },
      );
      console.log('[keevz] Slash Komutlar yüklendi.');
    } catch (error) {
      console.error(error);
    }
  })();
